...
const ReservationDetails = ({ 
  ...
  const [paymentDueDate, setPaymentDueDate] = useState(today);  // Agregar estado para paymentDueDate
      ...
      if (currentReservationId) {
        axios.get(`http://localhost:5000/api/reservations/${currentReservationId}`)
          .then(response => {
            ...
            setPaymentDueDate(reservation.payment_due_date || today); // Establecer la fecha límite si existe
            onPaymentDueDateChange(reservation.payment_due_date || today); // Notificar al componente padre
  ...
  const handlePaymentDueDateChange = (e) => {
    ...
    onPaymentDueDateChange(e.target.value); // Notificar al componente padre cuando cambie
  };
        ...
        {selectedClient && selectedBodega && (
          <Grid item xs={12} sm={6}>
            ...
            <TextField
              ...
              onChange={handlePaymentDueDateChange} // Controlar el cambio de la fecha
              size="small"
              fullWidth
              sx={{ marginTop: 2 }}
            />
