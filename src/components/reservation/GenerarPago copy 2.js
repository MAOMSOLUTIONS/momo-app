...
const GenerarPago = ({ selectedClient, selectedBodega, onPaymentUpdate, reservationId, paymentDueDate }) => {
    ...
    if (currentReservationId) {
      ...
      axios.get(`http://localhost:5000/api/reservations/${currentReservationId}`)
        .then(response => {
          ...
          setPaymentDueDate(reservation.payment_due_date || ''); // Asignar el valor de paymentDueDate si existe
  ...
  const handlePaymentSubmit = async () => {
    try {
      ...
      const dataToSend = {
        ...
        payment_due_date: paymentDueDate,  // Enviar la fecha límite de pago
      };
      // Imprime los datos que se enviarán al backend
      console.log("Datos que se envían al backend para crear la reservación:", dataToSend);
