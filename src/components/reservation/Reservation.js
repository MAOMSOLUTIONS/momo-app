...
import CreateEnterpriseForm from './CreateReservationForm';
import EnterpriseList from './ReservationList';
import BasicDataEnterpriseForm from './BasicDataReservationForm';
    ...
    <div>
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Typography>{t('Consulta de Reservaciones')}</Typography>
        </AccordionSummary>
        <AccordionDetails>
      ...
      <Accordion expanded={isCreateAccordionOpen} onChange={() => setIsCreateAccordionOpen(!isCreateAccordionOpen)}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} ref={modifyUserRef}>
          <Typography>{!selectedUser ? t("Crear Reservación") : t("Modificar Reservación")}</Typography>
        </AccordionSummary>
        <AccordionDetails>
          <div style={{ width: '100%' }}>
            {selectedUser !== null ? (
