...
const BasicDataForm = ({ onUserUpdated,initialValues, setSelectedUser, isFieldsEnabled, onClear  }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isCreating ? 'http://127.0.0.1:5000/api/users' : `http://127.0.0.1:5000/api/users/${formValues.id_user}`;
    ...
    try {
      const birthDateInput = formValues.birth_date;
      ...
      if (parts.length === 3) {
        ...
        // Verifica si el formato convertido es válido
        const datePattern = /^(\d{2})\/(\d{2})\/(\d{4})$/;
        if (!datePattern.test(formattedBirthDate)) {
          setSnackbarMessage('Formato de fecha no válido');
          setSnackbarSeverity('error');
          setOpenSnackbar(true);
          return;
        }
        ...
        // Realiza una solicitud POST para crear o editar el usuario
        // Reemplaza la URL y el método POST con la configuración correcta para tu API
...
//          handleClearFields();
  ...
  const getAccordionLabel = () => {
    return isCreating ? 'Crear Usuario' : 'Modificar Usuario';
  };
